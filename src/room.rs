...
impl Room {
    ...
    /// Add a producer to the room, this will trigger notifications to other participants that
    ...
    /// Get all producers of all participants, useful when a new participant connects and needs to
    ...
    /// Subscribe to notifications when a new producer is added to the room
    ...
    /// Subscribe to notifications when a producer is removed from the room
    ...
    /// Subscribe to notification when a room is closed
    pub fn on_close<F: FnOnce() + Send + 'static>(&self, callback: F) -> HandlerId {
        self.inner.handlers.close.add(Box::new(callback))
    }

    /// Get `WeakRoom` that can later be upgraded to `Room`, but will not prevent a room from
    /// being destroyed
...
/// Similar to `Room`, but doesn't prevent a room from being destroyed
#[derive(Debug, Clone)]
pub struct WeakRoom {
    inner: Weak<Inner>,
}
