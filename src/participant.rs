...
pub struct ParticipantConnection {
    ...
    room: Room,
    /// Event handlers that were attached and need to be removed when a participant connection is
    /// destroyed
...
impl Actor for ParticipantConnection {
    ...
    fn started(&mut self, ctx: &mut Self::Context) {
        ...
        // information about both in an initialization message alongside router
        ...
        // Listen for producers removed from the room
        ...
        // Notify a client about any producers that already exist in the room
    ...
    fn handle(&mut self, message: ClientMessage, ctx: &mut Self::Context) {
        match message {
            ClientMessage::Init { rtp_capabilities } => {
                // We need to know the client's RTP capabilities, those are sent using
                // an initialization message and are stored in connection struct for future use
                ...
                actix::spawn(async move {
                        ...
                        .await
                    {
                        Ok(producer) => {
                            ...
                            // Add a producer to the room so that others can consume it
                            room.add_producer(participant_id, producer.clone());
                            // Producer is stored in a hashmap since if we don't do it, it will
                            // get destroyed as soon as its instance goes out of scope
...
/// connection struct, and having such a message handler makes it easy to use from background tasks
    ...
    fn handle(&mut self, message: InternalMessage, ctx: &mut Self::Context) {
        match message {
            ...
            InternalMessage::SaveProducer(producer) => {
                // Retain a producer to prevent it from being destroyed
