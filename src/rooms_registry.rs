...
pub struct RoomsRegistry {
    ...
    // being destroyed when the last participant disconnects
    rooms: Arc<Mutex<HashMap<RoomId, WeakRoom>>>,
}

impl RoomsRegistry {
    /// Retrieves an existing room or creates a new one with a specified `RoomId`
    ...
    fn add_close_handler(&self, room: &Room) {
        room.on_close({
            let room_id = room.id();
